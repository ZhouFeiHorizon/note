...
- 缓存: webpack5 内置了`cache`模块、`cache-loader`, `HardSourceWebpackPlugin`
- 构建缩小范围 `inlude`,`exclude`
...
- `file-loader`, `url-loader`（小于多少的能转换成 base64）
- `image-loader`
- `svg-sprite-loader`
- `thread-loader`
...
- `uglifyjs-webpack-plugin`、`terser-webpack-plugin`、`HappyPack`
- `HardSourceWebpackPlugin` 缓存
- `speed-measure-webpack-plugin` 打包构建速度分析
...
8. 图片优化、图片压缩
9. webpack 按需加载代码，`hash`，`contenthash`
10. 减少重排重绘
11. 降低 css 选择器的复杂性
