...
- `Object.getOwnProperDescript(obj:object, key: string | number):object`： 获取指定对象的自身属性描述符。自身属性描述符是指直接在对象上定义（而非从对象的原型继承）的描述符。
  ...
  - 為了更高效的更新dom， 通過key判斷是否是同一個節點，寵兒避免頻繁更新不同原因， 使得整個patch過程更加高效，減少dom操作量，提高性能

- Vue key 的作用，為了更高效的更新dom，通过key可以精准判断两个节点是否是同一个，从而避免频繁更新不同元素，使得整个patch过程更加高效，减少dom操作量，提高性能。 vue为了更快的去渲染元素，就会复用一些元素，没有key时会渲染快一点
